...
[AGENTS.md](AGENTS.md) に、汎用的な「cron の統合と正確な実行時刻の管理」ルールを追加しています。これは独立した Skill ではなく、リポジトリで cron の設計・変更を扱う際の運用ルールです。クローラーに限らず、同期・通知・定期生成・キュー処理にも適用できます。

関連ジョブの共通入口、必要最小限の起動時刻、正確な時刻分岐、外部処理前の SKIP、週あたりの起動と実作業の区別、バックアップとインストール後の読み戻しを定めています。既存プロジェクトへ採用する場合は、そのプロジェクトの AGENTS.md に用途・権限・遅延要件を合わせて取り込みます。既存の `cron-crawler-safety` は、従来どおりクローラーの安全な実行を担当します。
