...
description: Secure server-side API integrations on Sakura Server. Use when Codex needs to centralize API keys outside the public web root, standardize environment-variable resolution across PHP/Python/cron, protect every browser-facing API with login/page permission/CSRF or scoped bearer authentication, prevent SSRF in URL-fetching proxies, migrate legacy key files, deploy through an exact SFTP allowlist, and verify production without exposing secrets or consuming unnecessary provider quota.
---
...
- Keep one managed credential store outside the public web root.
- Set its parent directory to `0700` and the file to `0600`.
- Use one canonical environment-variable name per provider. Do not add aliases or one key file per provider.
- Resolve credentials in this order: process environment, managed private store, explicit unconfigured result.
- Keep the same allowlist and resolution order in PHP, Python, and cron shell readers.
- Expose only the canonical variable name and a safe source label such as `environment`, `managed_private_config`, or `unconfigured`.
...
3. Define the canonical credential allowlist and private store. Read `references/credential-store.md`.
...
8. Migrate credentials before deploying readers that depend on the new store.
9. Build locally, lint every changed server file, deploy by allowlist, then run low-frequency production verification.
10. Review provider usage and server logs when an endpoint was previously public. Rotate a credential only when exposure or abuse is confirmed, or when the provider's policy requires it.
...
- Confirm the secret directory/file modes without printing contents.
